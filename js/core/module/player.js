...
Player = (function(Events) {
	...
	$.extend(true, Player, Events, {
		...
		init: function(config) {
			...
			/**
			 * @property {Number} height footerHeight height
			 */
			this.playbackFooter = this.config.playbackFooter;
			/**
		...
		initNative: function() {
            ...
            this.el.addEventListener('progress', function() {
                scope.onBuffer(scope.el.buffered);
            });
		...
		native: function(cmd, attrs) {
			...
			if (cmd === 'play') {
				if (typeof attrs.url !== 'undefined' && this.el.src !== this.url) {
					...
					this.hls.loadSource(this.el.src);
				...
				if (attrs.direction == 'up') {
					this.currentVolume = this.currentVolume + (attrs.percent / 100);
				} else {
					this.currentVolume = this.currentVolume - (attrs.percent / 100);
				}
				...
				this.el.volume = this.currentVolume;
			}
		},
		/**
		 * @private
        ...
        onBuffer: function(buffered) {
            this.trigger('buffer', buffered);
        },
		/**
		 * @private
		 */
		...
		fullscreen: function() {
			...
			return this.show(wwidth, wheight, 0, 0);
		},
		fitscreen: function() {
			var wwidth = $(document).width();
			var wheight = $(document).height();
			this.isFullscreen = false;
            var progressBarHeight = 11;
            this.trigger('fitscreen');
			return this.show(wwidth, wheight - this.playbackFooter.height() + progressBarHeight, 0, 0);
		},
		/**
		 * Set media URL
		 *
		...
		stop: function() {
            ...
            this.trigger('statechange', this.STATE_IDLE);
			this.reset();
		},
		/**
		 * Seek playback
		...
		volume: function (direction, percent) {
			var attibutes = { percent: percent, direction: direction };
			this.native('volume', attibutes);
			this.trigger('volume', attibutes);
		}
